...
-- Main Query with Inventory Filter
SELECT 
    ...
    CASE 
        ...
        WHEN ia.[WIP_Qty] < wip.[WIP_minimum] THEN
            (wip.[WIP_minimum] - ia.[WIP_Qty]) * ic.[Standard Cost]
        ELSE 0
...
LEFT JOIN
    DemandSummary ds
ON
    cp.[pt_part] = ds.[BOM_Component]
    AND cp.[pt_site] = ds.[SO_Site]
-- Filter to exclude items with zero inventory
WHERE COALESCE(ii.[Total Inv], 0) <> 0
ORDER BY 
