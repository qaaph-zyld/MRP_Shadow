...
WHERE
    th.[tr_type] = 'rct-wo'
    AND CAST(th.[tr_effdate] AS DATE) IN (
        -- First possible date: Friday if today is Monday, otherwise yesterday
        CASE 
            WHEN DATEPART(weekday, GETDATE()) = 2 THEN DATEADD(day, -3, CAST(GETDATE() AS DATE)) -- If today is Monday (weekday=2), get Friday (-3 days)
            ELSE DATEADD(day, -1, CAST(GETDATE() AS DATE)) -- Otherwise, get yesterday (-1 day)
        END,
        -- Second possible date: Saturday if today is Monday, otherwise NULL (which will be ignored)
        CASE 
            WHEN DATEPART(weekday, GETDATE()) = 2 THEN DATEADD(day, -2, CAST(GETDATE() AS DATE)) -- If today is Monday (weekday=2), get Saturday (-2 days)
            ELSE NULL -- Otherwise, this value is NULL and doesn't affect the IN clause
        END
    )
GROUP BY
    th.[tr_part],
