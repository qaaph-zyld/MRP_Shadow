...
PODData AS (
    ...
    WHERE pod.[pod_end_eff[1]]] = '2049-12-31'
),
NissanRatioData AS (
    SELECT 
        [dtcov_ifs] as [Item Number],
        CASE 
            WHEN [dtcov_right] = 1 AND [dtcov_left] = 1 THEN 1
            ELSE 0.5 
        END AS [Nissan WH ratio]
    FROM 
        [TCIS_MES2_265].[dbo].[dtcov_mstr]
    WHERE 
        [dtcov_cust_name] = 'JC SUNDERLAND' AND [dtcov_last] = 1
),
ComputedData AS (
    SELECT 
        MainData.[Item Number],
        PODData.[po_vend],
        MainData.[Prod Line],
        MainData.[FG Type],
        MainData.count_ser_serial_id,
        CASE
            ...
            ELSE NULL
        END * COALESCE(NissanRatioData.[Nissan WH ratio], 1) AS [Warehouse_Packaging_Ratio],
        CASE
        ...
        CASE
            ...
            WHEN MainData.[Prod Line] IN ('E_FG','R_FG','U_FG') AND MainData.[FG Type] <> 'other' THEN 'CV'
    ...
    LEFT JOIN 
        NissanRatioData
        ON MainData.[Item Number] COLLATE SQL_Latin1_General_CP1_CI_AS = NissanRatioData.[Item Number] COLLATE SQL_Latin1_General_CP1_CI_AS
)
SELECT 
    [WH_Group],
    [Stackability],
    [Warehouse_Packaging_Ratio],
    SUM(count_ser_serial_id) AS [Total Count],
    FLOOR(
        SUM(
            CASE 
                WHEN [Stackability] IS NULL OR [Stackability] = 0 THEN NULL
                ELSE (count_ser_serial_id * [Warehouse_Packaging_Ratio]) / [Stackability]
            END
        )
    ) AS [WH_Spot]
FROM 
