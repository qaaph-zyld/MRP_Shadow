...
-- Final SELECT with all columns from first query plus weekly demand and calculated columns (distinct rows)
SELECT DISTINCT 
    FQR.*,
    CDW.[Past_Due],
    CDW.[Current_Week],
...
LEFT JOIN
    (
        SELECT
            ...
            [ld_loc] as [Location],
            SUM([ld_qty_oh]) as [Qty per location],
            MAX([ld_status]) as [Inv Status]
        FROM [QADEE2798].[dbo].[ld_det]
        GROUP BY [ld_part], [ld_loc]
    ) loc
ON 
    FQR.[Item Number] = loc.[Item Number]
ORDER BY 
    FQR.[Item Number];