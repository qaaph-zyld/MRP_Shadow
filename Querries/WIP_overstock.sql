/*
WIP Overstock Analysis Report (2798 Only)
------------------------------------------
Identifies and analyzes items with WIP inventory outside the optimal range.
Cleaned for single-plant (2798) operation.

Purpose:
- Identify items with WIP inventory exceeding maximum or below minimum thresholds
- Quantify the financial impact of WIP overstock or understock
- Support inventory optimization initiatives

Business Rules:
- WIP_minimum = AVG(last 4 weeks ISS-WO) × 3
- WIP_maximum = AVG(last 4 weeks ISS-WO) × 7
- WIP_overstock = quantity outside [WIP_minimum, WIP_maximum] range
*/
...
-- Main query: items with WIP inventory outside optimal range
SELECT
    cp.[pt_site] AS [Plant],
    ...
    ia.[WIP_Qty],
    CASE 
        WHEN ia.[WIP_Qty] > wip.[WIP_maximum] THEN ia.[WIP_Qty] - wip.[WIP_maximum]
        WHEN ia.[WIP_Qty] < wip.[WIP_minimum] THEN ia.[WIP_Qty] - wip.[WIP_minimum]
        ELSE NULL
    END AS [WIP_overstock],
    CASE 
        WHEN ia.[WIP_Qty] > wip.[WIP_maximum] THEN (ia.[WIP_Qty] - wip.[WIP_maximum]) * ic.[Standard Cost]
        WHEN ia.[WIP_Qty] < wip.[WIP_minimum] THEN (ia.[WIP_Qty] - wip.[WIP_minimum]) * ic.[Standard Cost]
        ELSE 0
    END AS [WIP_overstock_Value],
    CASE 
        WHEN ia.[WIP_Qty] > wip.[WIP_maximum] THEN 'Over Maximum'
        WHEN ia.[WIP_Qty] < wip.[WIP_minimum] THEN 'Under Minimum'
        ELSE 'In Range'
    END AS [WIP_Status],
...
WHERE
    ...
    AND wip.[WIP_minimum] IS NOT NULL
    AND ia.[WIP_Qty] IS NOT NULL
    AND ii.[Total Inv] > 0
ORDER BY 
    ABS(CASE 
        WHEN ia.[WIP_Qty] > wip.[WIP_maximum] THEN (ia.[WIP_Qty] - wip.[WIP_maximum]) * ic.[Standard Cost]
