...
DetailedData AS (
    SELECT
        ...
        -- Renamed bkfl_qty to qty_per_boxes
...
-- CTE 4: Get work order transaction data (iss-wo and rct-wo)
WorkOrderData AS (
    SELECT
    ...
    GROUP BY
        ...
        [tr_prod_line]
    HAVING
        SUM(CASE WHEN [tr_type] = 'iss-wo' THEN [tr_qty_loc] ELSE 0 END) <> 0
        OR
        SUM(CASE WHEN [tr_type] = 'rct-wo' THEN [tr_qty_loc] ELSE 0 END) <> 0
),
-- CTE 5: Join the data and create the RM/SFG/FG column in preparation for the final calculation
PreFinalData AS (
    SELECT
        a.Item_Number,
        w.[tr_prod_line],
...
-- Final Step: Select from the PreFinalData CTE and apply the conditional delta calculation
SELECT
    Item_Number,
    [tr_prod_line] AS [Prod Line],
    [RM/SFG/FG],
    qty_per_boxes,
    SerialID_Count,
    active_qty,
